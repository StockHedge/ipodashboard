...
## 22. v1.9 (2026-06-16) — 의무보유확약(락업) 구간별 해제일 (6개월 일괄 폐기)

종전: 전 종목 listingDate+6개월 단일 만료로 퉁침. 실제 기관 의무보유확약(=락업=확약)은
15/30/90/180일 구간으로 분산 해제됨 → 종목별·구간별 해제일을 각각 산출하도록 재구조.

- 핵심 모델: `LOCKUP_PERIODS`(15/30/90/180일) + `getLockupDate(r,days)` + `getLockupDday(r,days=180)`
  + `getLockupReleases(r)`(4구간 [{days,label,date,dday}]). `getLockupExpiry`는 180일 별칭(호환).
- 락업 캘린더: '○일 이내' 윈도 → **구간 선택기**(15/30/90/180일). 선택 구간 해제일이 다가오는
  종목을 D-day 순 표시(최근 상장 코호트 자연 노출). 클릭 시 종목 상세.
- 종목 상세: '의무보유확약 해제 일정' 카드 신설 — 4구간 해제일·D-day(해제됨 표기) 각각.
- 오늘 보드/알림: 4구간 중 임박(14일/7일 이내) 건을 '락업해제 (구간)' 으로 각각 노출.
- KPI '확약해제 임박': 4구간 중 하나라도 30일 이내면 카운트.
- 검증: 피스피스(6/08) 15일 D-7·30일 D-22·90일 D-82·180일 D-172, 마키나락스(5/20) 30일 D-3 임박.
- 한계: 보유 데이터의 lockupRate 는 '총 확약 비율' 단일 값 → 구간별 비율(15일 X%·30일 Y%…)은
  미수록. 구간별 '해제일·D-day' 는 정확, 구간별 '비율' 은 별도 데이터(DART/38 상세) 필요.

## 23. v1.10 (2026-06-19) — 신규상장 소스 교체: 38 아카이브(o=nw/o=r1) → 누락 종목·확약비율 확보

사용자 지적("15/30일 락업 임박 종목이 없을 리 없다") → 직접 웹 서칭으로 근본원인 규명:
- 이전 파이프라인은 38 *진행중*(fund.php) + fdr 이름매칭에 의존 → 상장 후 롤오프된 종목·스팩을 누락.
  실측 결과 메리츠스팩2호(6/19)·대신밸런스스팩20호(6/05, 15일 해제 D-1) 등 통째로 빠져 있었음.
- 네이버 '신규상장' 페이지는 전부 ETN/ETF (실제 기업 IPO 0건) — 부적합 확인.
- 38 데스크탑 o=nw(신규상장 아카이브: 상장 후에도 유지)·o=r1(수요예측결과)이 정답.
  Python 기본 ssl 이 38 레거시 cipher 로 handshake 실패 → SECLEVEL=1 컨텍스트 우회(검증 우선·CERT_NONE fallback).

listings_client.py 재작성(v2):
- o=nw: 상장일·확정공모가·첫날 시초/종가 수익률 (상장 완료행만, 컬럼 안정).
- o=r1: 기관경쟁률 + **의무보유확약 비율(lockupRate)** + 주간사 + 밴드 — 그동안 비어있던 필드 확보.
- fdr: 종목코드/시장 보조(선택, SPAC 미매칭 시 KOSDAQ). KIS 의존 제거(첫날수익률 o=nw 직접).
- 결과 4건 → 6건 (since 4/30 전체), 확약비율·경쟁률 포함. 15/30일 락업 뷰가 정상 노출됨.
- 한계: 확약은 총 비율만 (구간별 15/30/90/180 % 는 종목 상세/DART 필요). 해제일은 상장일로 정확 산출.

## 24. v1.10.1 (2026-06-19) — 락업 표시 정리: 비율 제거, 순수 날짜 기반

사용자 지적: 확약을 '비율'로 표시할 이유 없음 — 핵심은 상장일 + 15/30/90/180일 해제일.
- 락업 캘린더·종목 상세에서 '총 확약 %' 표시 전부 제거. 표시는 상장일·해제일·D-day 만.
- lockupRate 데이터는 유지하되 화면 비표시(AI 점수 신호로만 사용).
- 해제 일정은 전적으로 상장일 기준 산정 (상장일은 38 o=nw 에서 확보).
